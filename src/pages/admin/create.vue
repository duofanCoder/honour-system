      ...
      <el-scrollbar height="840px">
        ...
        <div class="flex flex-row h-full mt-5">
          ...
          <div class="bg-blue h-full w-1/2">
            ...
            <div>
              <el-tag
                >上传的第一张照片会作为荣誉推荐的第一张图片,上传文件仅支持jpg, jpeg, png
                最多支持6个文件</el-tag
              >
            </div>
          </div>
          ...
          <div class="w-1/2">
            ...
            >
              ...
              <el-form-item label="获得者">
                <el-button class="my-0" type="primary" @click="handleTabAction('add')"
                  >添加</el-button
                >
                <el-table :data="actTableData" style="width: 700px; height: 600px">
                  ...
                  <el-table-column label="操作">
                    <template #default="{ row }">
                      <div class="w-full flex space-x-0">
                        <el-button size="small" @click="handleTabAction('remove', row)"
                          >删除</el-button
                        >
                      </div>
                    </template>
...
<script setup lang="ts">
  ...
  const handleImg2Word = async () => {
    ...
    rightDrawerRef.value.acceptParams({
      drawer: true,
      rowData: await getTextFromImage(imageUrl.value),
    });
  };
  ...
  const rules = {
    ...
    levelId: [{ required: true, message: '请选择荣誉级别', trigger: 'blur' }],
    categoryId: [{ required: true, message: '请选择荣誉类别', trigger: 'blur' }],
    actTime: [{ required: true, message: '请选择获取时间', trigger: 'blur' }],
    term: [{ required: true, message: '请输入获奖学期', trigger: 'blur' }],
  };
  ...
  const handleTabAction = (action: string, row?: any) => {
    switch (action) {
      ...
      case 'remove':
        // 删除actTableData
        actTableData.value.splice(
          actTableData.value.findIndex((item: any) => item.id === row.id),
          1
        );
        break;
    }
